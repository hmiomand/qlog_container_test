...
fn prepare_tiles(images_folder: &str, tile_size: &Size, verbose: bool) -> Result<Vec<RgbImage>, Box<dyn Error>> {
    ...
    for image_path in image_paths {
        ...
        pool.execute(move || {
            ...
            // let tile: RgbImage = ImageReader::open(image_path?.path())?.decode()?.into_rgb8();
...
unsafe fn l1_x86_avx2(im1: &RgbImage, im2: &RgbImage) -> i32 {
	// Suboptimal performance due to the use of _mm256_loadu_si256, still better than SSE2.
    use std::arch::x86_64::{
        __m256i,
    ...
    for i in (0..nb_sub_pixel - stride).step_by(stride as usize) {
        // Get pointer to data
        let p_im1: *const __m256i = std::mem::transmute::<*const u8, *const __m256i>(std::ptr::addr_of!(im1[i as usize]));
        let p_im2: *const __m256i = std::mem::transmute::<*const u8, *const __m256i>(std::ptr::addr_of!(im2[i as usize]));
